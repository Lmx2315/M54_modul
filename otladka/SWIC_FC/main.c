...
volatile unsigned int SWIC_Speed = 20;//1 - 5 Mбит , 2 - 10 Мбит,8 - 40 Мбит, 0x50 - 400 Мбит/с
unsigned  int speed;
...
void x_out (const char *s,u32 a) //НЕ РАБОТАЕТ!!!
{
   Transf (s);
   itox (a,strng);
// sprintf (strng,"%X",a);
...
//-----------первый канал--------------
   ...
   for (i=0;i<63;i++) //Коэффициенты первого DFIR фильтра-дециматора
   {
	  adr=0x0040+i;
	  spi_1288_wr (adr,DSP.RX1_CF_DFIR1[i]);
   }

   for (i=0;i<63;i++) //Коэффициенты второго DFIR фильтра-дециматора
   {
	  adr=0x0080+i;
