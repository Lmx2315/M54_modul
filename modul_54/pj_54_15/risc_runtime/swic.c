...
enum ERL_ERROR swic_send_packet(int id, void * src, int size, int type)
{
  ...
  {
	txDescr0[0] = 0;
	txDescr0[1] = id; // & number port
	txDescr0[0] = 0x80000000 + size + (type<<29); 
  } else	  
  if (id==4)
  {
	txDescr1[0] = 0;
	txDescr1[1] = id; // & number port
	txDescr1[0] = 0x80000000 + size + (type<<29); 
  } else
	 if (id==6)
  {
	txDescr [0] = 0;
  ...
  unsigned int wsize = size >> 3;
   ...
   {
	// Prepare SWIC DMA channels
	IR_SWIC_TX_DES(id) = sys_kernel_va_to_pa((unsigned)txDescr0);   
   } else
   if (id==4)
   {
	// Prepare SWIC DMA channels
	IR_SWIC_TX_DES(id) = sys_kernel_va_to_pa((unsigned)txDescr1);   
   } else
	if (id==6)
   {
	// Prepare SWIC DMA channels
