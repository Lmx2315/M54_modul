...
volatile unsigned int SWIC_Speed = 10;//1 - 5 Mбит , 2 - 10 Мбит,8 - 40 Мбит,40 - 200 Мбит, 0x50 - 400 Мбит/с
unsigned  int speed;
  ...
  dsp->ADC_CFG = (0<<15)
   ...
   dsp->RX_CFG = (0<<15) //wr_all  | «1» соответствует одновременной записи во все каналы
	...
	dsp-> RX1_DCICN = (3<<14) //Биты конфигурации 2-й ступени CIC–дециматора:- «112» - 2-й каскад децимации включен в режиме CIC6
					 ...
					 |(10 );//Регулировка уровня выходного сигнала 2-й ступени CIC-дециматора:- если scl_mx = 1, то коэффициент передачи CIC–дециматора
	...
	dsp->RX1_DFIR1_CFG2 = (0<<4 ) //Зарезервировано
						 |(6);//Регулировка уровня выходного сигнала DFIR1.Коэффициент усиления на выходе DFIR1 равен 2scl-21
	...
	dsp->RX1_DFIR2_CFG2 = (0 <<4) //Зарезервировано
						 |(4);//Регулировка уровня выходного сигнала DFIR2.Коэффициент усиления на выходе DFIR2 равен 2scl-21
...
//-------------Для обновления внутренних значений необходимо записать «1» в бит SYNC_sync_gain.	--------------------------
	...
	dsp->RX1_GAIN_I = 0;//регистры установки комплексного коэффициента усиления (GAIN_I+j*GAIN_Q) Используемая кодировка – дополнение до двух
	dsp->RX1_GAIN_Q = 0x4000;

	dsp->RX2_GAIN_I = 0;//регистры установки комплексного коэффициента усиления (GAIN_I+j*GAIN_Q) Используемая кодировка – дополнение до двух
	dsp->RX2_GAIN_Q = 0x4000;

	dsp->RX3_GAIN_I = 0;//регистры установки комплексного коэффициента усиления (GAIN_I+j*GAIN_Q) Используемая кодировка – дополнение до двух
	dsp->RX3_GAIN_Q = 0x4000;

	dsp->RX4_GAIN_I = 0;//регистры установки комплексного коэффициента усиления (GAIN_I+j*GAIN_Q) Используемая кодировка – дополнение до двух
