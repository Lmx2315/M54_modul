 ...
 dsp->SYNC  = (1<<15) // «1»включается «Master» синхронизации
  ...
  dsp->ADC_CFG = (0<<15)
   ...
   dsp->RX_CFG = (0<<15) //wr_all  | «1» соответствует одновременной записи во все каналы
				...
				|(1<<0 );//in_fmt  | Формат входных данных каналов приема:Прямой  1 -offset binary
	...
	dsp-> RX1_DCICN = (3<<14) //Биты конфигурации 2-й ступени CIC–дециматора:- «112» - 2-й каскад децимации включен в режиме CIC6
					 ...
					 |(8 );//Регулировка уровня выходного сигнала 2-й ступени CIC-дециматора:- если scl_mx = 1, то коэффициент передачи CIC–дециматора
...
//	z=100000000-z;  //надо было с промежуткой 75 МГц
...
//	route = route_mask_spw0|route_mask_spw1;
	route = route_mask_spw1;

	if (a==0) swic_send_packet(2, OutputArray0    , size, 1);
	if (a==1) swic_send_packet(4, OutputArray1    , size, 1);
 ...
 while (1)
  {
	...
	if (SYS_timer2>1000) flag_DMA_LPORT=1; //временно для отладки на плате элвиса
		...
		for (i=1;i<N_col;i++)
		 {
			...
			} else
			{
				...
				accum_0 =((lport_InputArray[v1]>>16)&0xffff);// для 0 канала
				accum_1 =((lport_InputArray[v2]>>16)&0xffff);// для 1 канала
