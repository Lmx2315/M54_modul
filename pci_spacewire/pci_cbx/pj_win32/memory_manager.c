...
int init_manager ()
{
    ...
    for (i=0;i<SWIC_NUMBER;i++){
        ...
        manager.port[i].tx_speed =  50; //Stable speed for MCB02 50
...
int init_rx_dma (unsigned int num)
{
 ...
 mcb_write(SWIC_DMA_RX_DES_RUN(num) ,0x01);
 mcb_write(SWIC_DMA_RX_DATA_RUN(num),0x01);
 return res;
 }
...
unsigned int wsize = 0;   
 ...
 mcb_write(SWIC_DMA_RX_DES_CSR(index_port),(((MAX_RX_DESCRIPTORS_IN_MEMORY) << 16)));
